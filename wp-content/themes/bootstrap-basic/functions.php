...
/**
 * --------------------------------------------------------------
 * Ross added functions
 * --------------------------------------------------------------
 */
//when logout clicked both the blog site and main site should logout at same time
//loggout redirect to the main site
function auto_redirect_external_after_logout(){
    ...
    wp_redirect( 'http://www.unforgettable.me/logout' );
    exit();
}
add_action( 'wp_logout', 'auto_redirect_external_after_logout');

// function unset_parameter_after_logout() {
//     echo "string";
//     if (isset($_SESSION['email']) && isset($_SESSION['userid'])) {
//         unset($_SESSION['email']);
//         unset($_SESSION['userid']);
...
//     }
// }
// add_action('wp_logout', 'unset_parameter_after_logout');
...
//If server don't start session by default, start it manually
...
//Get email/userid from GET command after 'u_' prepending
//Start a timer in session
function ufgt_session_create() {
    if (isset($_GET['u']) && !empty($_GET['u'])) {  //check if the 'u' parameter is exist and not empty
        $u_str = $_GET['u'];
        $dec_str = base64_url_decode($u_str);
        parse_str($dec_str, $param);

        if(array_key_exists("email", $param) && array_key_exists("userid",$param)) {
